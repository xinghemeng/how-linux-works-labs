...
实验以 C 为实现基础，C++17 树用于系统编程练习。两套代码由 CMake 隔离为 `sched_c` 与 `sched_cpp`，互不混编。`sched_cpp/` 下的 `.cpp` / C++ 头文件以本文为准；C 代码以 `code_review_c.md` 为准。同一改动里两种语言并存时，按文件扩展名分别套用对应规范。
...
2. 本文是该规范在本项目中的**落地摘要与审查清单**，并补充本实验的开发约定。
3. 冲突处理：
...
- `thread_local` 仅在确实需要线程局部状态时使用，并文档化生命周期。本实验默认用**进程**而不是线程来产生并发负载；不要在未说明的情况下把 `n` 实现成 `n` 条线程。
...
本实验用多进程忙等消耗 CPU，并按固定 CPU 时间间隔采样。除 Google 规范外，审查必须检查：

1. **进程模型是进程，不是线程**  
   用 `fork`（或明确文档化且语义等价的方式）创建 `n` 个子进程。父进程 `waitpid`（或等价）回收**全部**子进程后再退出。检查 `fork` / `wait` 返回值；失败路径上已创建的子进程必须收尸，禁止留下僵尸进程。子进程记录里的 ID 是 `0` ~ `n-1` 的实验编号，不要和 OS `pid` 混用而不加说明。
...
4. **参数、溢出与进度**  
   校验 `n > 0`、`total > 0`、`resol > 0`，并处理 `resol > total`、不能整除等情况（策略写进注释或 `--help`，不要静默给出空输出）。毫秒换算用 64 位，`timespec` 的 `tv_sec` / `tv_nsec` 转毫秒要防溢出。进度用已消耗 CPU / `total`，最后一次采样应达到或明确逼近 100%。

5. **资源在所有路径上释放**  
   管道 fd、动态数组、子进程 PID 表，错误返回路径也要 `close` / `free` / `wait`。禁止只在 happy path 里清理。

6. **运行条件写清楚**  
   逻辑 CPU 个数会改变观测图像；绑核（如 `taskset`）是实验变量，不是隐式依赖。用法写在 `--help` 或 `docs/`：三个参数含义、输出三列格式、CPU 时间而非墙钟。不要把「必须看见完美 round-robin」写进断言——程序负责出数据，调度结论放在实验分析里。

7. **与 C 代码共存时**  
   C++ 源遵守本文；C 源遵守 `code_review_c.md`。不要在 `.cpp` 里按内核 Tab-8 去「统一」风格。
...
- [ ] 无悬空指针、重复释放、fd 泄漏；失败路径回收已 fork 的孩子
